...
public const val MILLIS_PATTERN = "yyyy-MM-dd HH:mm:ss.SSSXXX"
 ...
 * 昨天的星期，默认格式EE
 */
inline val <T> T.yesterdayWeek: String
    get() = SimpleDateFormat(WEEK_PATTERN, Locale.getDefault()).format(nowMillis - ONE_DAY_MILLIS)

/**
 * 明天的星期，默认格式EE
 */
inline val <T> T.tomorrowWeek: String
    get() = SimpleDateFormat(WEEK_PATTERN, Locale.getDefault()).format(nowMillis + ONE_DAY_MILLIS)
 ...
 */
inline val <T> T.nowDateTimeStr: String
 ...
 * 当前时间 标准格式的string，后续可以扩展更多time相关的属性和函数
 */
inline val <T> T.nowDateStr: String
    get() = SimpleDateFormat(DATE_PATTERN, Locale.getDefault()).format(nowMillis)

/**
 * 明天 标准格式的 日期样式 yyyy-MM-dd
 */
inline val <T> T.tomorrowDate: String
    get() = SimpleDateFormat(DATE_PATTERN, Locale.getDefault()).format(nowMillis + ONE_DAY_MILLIS)

/**
 * 后天 标准格式的 日期样式 yyyy-MM-dd
 */
inline val <T> T.afterTomorrowDate: String
 ...
 * 后天 标准格式的 日期样式 yyyy-MM-dd
 */
inline val <T> T.yesterdayDate: String
    get() = SimpleDateFormat(DATE_PATTERN, Locale.getDefault()).format(nowMillis - ONE_DAY_MILLIS)

/**
 * 后天 标准格式的 日期样式 yyyy-MM-dd
 */
inline val <T> T.beforeYesterdayDate: String
    get() = SimpleDateFormat(
        DATE_PATTERN,
        Locale.getDefault()
    ).format(nowMillis - 2 * ONE_DAY_MILLIS)
 ...
 * [millis] 时间戳 毫秒值
 */
fun <T> T.isBeforeYesterday(millis: Long): Boolean {
 ...
 * [millis] 时间戳 毫秒值
 */
fun <T> T.isYesterday(millis: Long): Boolean {
 ...
 * [millis] 时间戳 毫秒值
 */
fun <T> T.isToday(millis: Long): Boolean {
 ...
 * [millis] 时间戳 毫秒值
 */
fun <T> T.isTomorrow(millis: Long): Boolean {
    return nowDateStr == (millis - ONE_DAY_MILLIS).toDate()
}
 ...
 * [millis] 时间戳 毫秒值
 */
fun <T> T.isAfterTomorrow(millis: Long): Boolean {
    return nowDateStr == (millis - 2 * ONE_DAY_MILLIS).toDate()
}
 ...
 * [millis] 时间戳 毫秒值
 */
fun <T> T.getUsWeek(millis: Long): String {
 ...
 * [millis] 时间戳 毫秒值
 */
fun <T> T.getCnWeek(millis: Long): String {
 ...
 */
fun <T> T.toDateTimeStr(millis: Long, pattern: String = DATE_TIME_PATTERN): String {
 ...
 */
fun <T> T.getTimeSpan(source: Long, destination: Long = nowMillis): Int {
 ...
 */
private fun Long.toDate(): String = SimpleDateFormat(DATE_PATTERN, Locale.getDefault()).format(this)
